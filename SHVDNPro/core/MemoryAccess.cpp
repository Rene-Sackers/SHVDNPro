...
namespace GTA
{
	namespace Native
	{
		namespace
		{
			private ref struct EntityPoolTask
			{
				enum class Type
				{
					Ped,
			...
			private ref struct EuphoriaMessageTask
			{
				...
				virtual void Run()
				{
					...
					{
						IntPtr name = GTA::Native::Function::AddStringPool(argument.Key);
						...
						{
							MemoryAccess::SetNmStringAddress(MessageAddress, name.ToInt64(), GTA::Native::Function::AddStringPool((String ^)argument.Value).ToInt64());
						}
					}
					...
					if (*reinterpret_cast<__int64*>(_PedAddress + 48) == PedNmAddress && *reinterpret_cast<float*>(_PedAddress + 5232 + AddrOff) <= *reinterpret_cast<float*>(_PedAddress + 640))
					{
						if ((*reinterpret_cast<int(**)(__int64)> (*reinterpret_cast<__int64*>(PedNmAddress) + 152))(PedNmAddress) != -1)
						{
							...
							{
								reinterpret_cast<void(*)(__int64, __int64, __int64)>(*reinterpret_cast<int*>(BaseFunc + 0x1AA) + BaseFunc + 0x1AE)(PedNmAddress, GTA::Native::Function::AddStringPool(_message).ToInt64(), MessageAddress);//Send Message To Ped
							}
			...
			private ref struct GenericTask
			{
			public:
		...
		{
			IntPtr handle = GTA::Native::Function::AddStringPool(toHash);
		...
		{
			(gcnew EuphoriaMessageTask(targetHandle, message, arguments))->Run();
		}

		int MemoryAccess::CreateTexture(System::String ^filename)
		{
			return createTexture(static_cast<const char *>(GTA::Native::Function::AddStringPool(filename).ToPointer()));
		}
