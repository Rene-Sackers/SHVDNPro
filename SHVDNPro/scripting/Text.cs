...
namespace GTA.UI
{
	...
	public class Text : IElement
	{
		...
		void InternalDraw(SizeF offset, float screenWidth, float screenHeight)
		{
			...
			IntPtr bufferSub = Marshal.AllocCoTaskMem(maxStringSize + 1);
			for (int i = 0; i < buffer.Length; i += maxStringSize)
			{
				var sizeSub = System.Math.Min(maxStringSize, buffer.Length - i);

				Marshal.Copy(buffer, i, bufferSub, sizeSub);
				Marshal.WriteByte(bufferSub, sizeSub, 0);

				Function.Call(Hash.ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME, bufferSub);
			}
